...
/usr/bin/time -f "\n\tT> %E [h:]m:s.s\n\tM> %M KB" bwa mem -t `nproc` -x ont2d $index $inputfile > $outdir/$outputfile.bwa.sam
# /usr/bin/time -f "\n\tT> %E [h:]m:s.s\n\tM> %M KB" bwa mem -t `nproc` -x ont2d /opt/rica_s/tools/rica_s_id_bwa/all_pathogens.fasta /opt/rica_s/datasets/_1/amr_EreA.fasta > ./outputfile.bwa.sam
